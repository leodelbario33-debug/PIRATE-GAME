...
export function buildRacer(def: CraftDef): PlayerRig {
  const g = new THREE.Group();
  // casco único MUY LARGO: popa ancha que se afina hacia la proa (40 m de eslora)
  const segs: [number, number, number, number][] = [
    // [ancho, profundidad, centroZ, altoY]
    [7.4, 8, -12, 1.25],
    [6.4, 8, -4, 1.25],
    [5.0, 8, 4, 1.2],
    [3.4, 7, 11.5, 1.1],
  ...
  for (const [w, d, zc, h] of segs) {
    g.add(box(w, h, d, M.hullDark, 0, h / 2 - 0.15, zc));
    // franja de carreras a ambos costados del segmento
    for (const s of [-1, 1]) g.add(box(0.16, 0.3, d + 0.05, M.orange, s * (w / 2 - 0.05), h * 0.62, zc));
  }
  // punta de proa afilada
  const bowTip = new THREE.Mesh(new THREE.CylinderGeometry(0.01, 1.1, 5, 4, 1), M.hullDark);
  bowTip.rotation.x = Math.PI / 2;
  bowTip.rotation.y = Math.PI / 4;
  bowTip.scale.set(0.85, 1, 0.5);
  bowTip.position.set(0, 0.45, 22.4);
  g.add(bowTip);
  // piso de cubierta continuo (se camina sobre él de popa a proa)
  g.add(box(6.9, 0.3, 32, M.deckSteel, 0, 1.28, 2));
  g.add(box(7.0, 0.08, 32.1, M.black, 0, 1.1, 2));
  // cabina baja en la zona ancha de popa
  g.add(box(3.4, 0.9, 3.2, M.dark, 0, 1.95, -9));
  g.add(box(3.2, 0.6, 0.14, M.windowBlue, 0, 2.35, -7.42));
  // jaula antivuelco
  for (const s of [-1, 1]) g.add(box(0.12, 0.85, 0.12, M.steel, s * 1.5, 2.6, -9.6));
  g.add(box(3.1, 0.12, 0.12, M.steel, 0, 3.0, -9.6));
  // alerón trasero ancho sobre los motores
  g.add(box(7.2, 0.14, 1.3, M.orange, 0, 2.75, -14.4));
  for (const s of [-1, 1]) g.add(box(0.16, 1.0, 0.5, M.dark, s * 2.7, 2.2, -14.4));
  // toma de aire central
  g.add(box(1.2, 0.6, 1.8, M.black, 0, 1.9, -5.2));
  // 8 motores de 800 HP en fila en la popa ancha
  const enginePuffs: THREE.Object3D[] = [];
  for (let i = 0; i < 8; i++) {
    const e = new THREE.Group();
    e.add(box(0.72, 1.45, 1.9, M.black, 0, 0.7, 0));
    e.add(box(0.62, 0.55, 1.6, M.orange, 0, 1.6, -0.1));
    e.add(box(0.3, 0.9, 0.3, M.steel, 0, -0.25, -0.55));
    const px = -2.98 + i * 0.85;
    e.position.set(px, 0.35, -16.6);
    g.add(e);
    const puff = new THREE.Object3D();
    puff.position.set(px, 0.25, -17.9);
    g.add(puff);
    enginePuffs.push(puff);
  }
  // torreta compacta hacia proa
  const turret = new THREE.Group();
  turret.position.set(0, 1.75, 10);
  ...
  turret.add(muzzle);
  g.add(turret);
  const bowAnchor = new THREE.Object3D(); bowAnchor.position.set(0, 0.5, 24); g.add(bowAnchor);
  const sternAnchor = new THREE.Object3D(); sternAnchor.position.set(0, 0.3, -18.5); g.add(sternAnchor);
  return { group: g, turret, muzzle, enginePuffs, bowAnchor, sternAnchor };
}
