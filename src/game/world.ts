...
export function buildRacer(def: CraftDef): PlayerRig {
  ...
  for (const s of [-1, 1]) {
    const hull = new THREE.Mesh(new THREE.BoxGeometry(2.3, 1.15, 17), M.hullDark);
    hull.position.set(s * 2.6, 0.55, 0.6);
    g.add(hull);
    // proa inclinada de cada casco
    const bow = new THREE.Mesh(new THREE.CylinderGeometry(0.01, 1.15, 4.8, 4, 1), M.hullDark);
    bow.rotation.x = Math.PI / 2;
    bow.rotation.y = Math.PI / 4;
    bow.scale.set(1, 1, 0.62);
    bow.position.set(s * 2.6, 0.5, 11.3);
    g.add(bow);
    // franja de carreras naranja
    g.add(box(2.35, 0.26, 17.1, M.orange, s * 2.6, 0.98, 0.6));
    // patín interior blanco
    g.add(box(0.5, 0.3, 15.4, M.white, s * 1.55, 0.45, 0.2));
  }
  // piso de cubierta: losa ancha que une los dos cascos (se camina sobre ella)
  g.add(box(7.8, 0.34, 14.6, M.deckSteel, 0, 1.12, 0.4));
  g.add(box(7.9, 0.1, 14.7, M.black, 0, 0.93, 0.4));
  // cubierta-puente central ancha
  g.add(box(4.6, 0.55, 12.6, M.deckSteel, 0, 1.5, 0.2));
  // cabina baja de carreras con parabrisas
  g.add(box(3.1, 0.85, 2.5, M.dark, 0, 2.15, 2.6));
  g.add(box(2.9, 0.55, 0.14, M.windowBlue, 0, 2.5, 3.82));
  // jaula antivuelco
  for (const s of [-1, 1]) g.add(box(0.12, 0.8, 0.12, M.steel, s * 1.35, 2.75, 2.2));
  g.add(box(2.8, 0.12, 0.12, M.steel, 0, 3.15, 2.2));
  // alerón trasero grande
  g.add(box(6.6, 0.14, 1.2, M.orange, 0, 2.6, -8.2));
  for (const s of [-1, 1]) g.add(box(0.16, 0.9, 0.5, M.dark, s * 2.4, 2.1, -8.2));
  // toma de aire central
  g.add(box(1.1, 0.6, 1.6, M.black, 0, 2.0, -2.4));
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const px = -2.8 + i * 0.8;
    e.position.set(px, 0.35, -9.2);
    g.add(e);
    const puff = new THREE.Object3D();
    puff.position.set(px, 0.25, -10.4);
    g.add(puff);
    enginePuffs.push(puff);
  }
  // torreta compacta a proa
  const turret = new THREE.Group();
  turret.position.set(0, 1.85, 5.6);
  ...
  turret.add(muzzle);
  g.add(turret);
  const bowAnchor = new THREE.Object3D(); bowAnchor.position.set(0, 0.5, 13.6); g.add(bowAnchor);
  const sternAnchor = new THREE.Object3D(); sternAnchor.position.set(0, 0.3, -11.2); g.add(sternAnchor);
  return { group: g, turret, muzzle, enginePuffs, bowAnchor, sternAnchor };
}
